...
ADMIN_KEY = os.environ.get("COINNECT_ADMIN_KEY", "coinnect-admin-2026")
...
# ── Public: suggestions (fingerprint-gated) ───────────────────────────────────
...
async def upvote_suggestion(sid: int, body: UpvoteBody):
    if not body.fingerprint or len(body.fingerprint) < 4:
        raise HTTPException(400, "fingerprint required")
